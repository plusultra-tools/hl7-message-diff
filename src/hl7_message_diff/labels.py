"""Embedded HL7 v2.6 segment field labels.

Source: HL7 v2.6 standard, chapters 2 (control), 3 (patient administration),
4 (order entry), 7 (observation). Field positions are 1-based.

Lookup: LABELS[segment_name][field_position] -> human-readable name.
Falls back to f"{segment}-{position}" when not found.
"""
...
def label_for(segment: str, field: int) -> str:
    """Return human-readable label for a given segment/field position.

    Falls back to f"{segment}-{field}" if no embedded label exists.
    """
    seg_labels = LABELS.get(segment, {})
    name = seg_labels.get(field)
    if name is None:
        return f"{segment}-{field}"
    return f"{segment}-{field} {name}"
